...
function reload() {
    window.location.href = '/admin/unlistedProducts'
}

// get data to unlist

function getDataTolist(id, name) {
    document.getElementById('name_').innerHTML = name
    const confirmButton = document.getElementById('confirm_btn');
    confirmButton.setAttribute('onclick', `listproduct('${id}')`);
}
...
async function listproduct(id) {
    ...
    try {
        ...
        const resp = await fetch(`/admin/product/list`, {
            method: 'PATCH',
            headers: {
                'Content-type': 'application/json'
            },
