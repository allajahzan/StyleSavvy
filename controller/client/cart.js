...
exports.getCartPage = async (req, res) => {
    try {
        ...
        return res.render('cart', { user, cart: itemsToDisplay, items: cart[0].items, total: result.totalPrice, count: result.count });
    } catch (err) {
        console.log(err);
        res.render('500')
    }
...
exports.addToCart = async (req, res) => {
    ...
    try {
        ...
        if (!cart) {
            ...
            await newCart.save();
            return res.status(200).json({ msg: 'Successfully added to cart', type: 'success' });
        }
        ...
        if (isItemAvailable) {
            return res.status(200).json({ msg: 'Already added to cart', type: 'success' });
        }
...
exports.removeItem = async (req, res) => {
    try {
        ...
        res.status(200).json({ msg: 'Successfully removed the item', items: cart[0].items, type: 'success', total: result.totalPrice, count: result.count })
