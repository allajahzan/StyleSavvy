...
exports.sendLinkToEmail = async (req, res) => {
    try {
        ...
        const user = await User.findOne({email:email, isGoogleAuthenticated:true})
        if(user){
            return res.status(401).json({msg:'This email is authenticated through google,so you can\'t change the password', type:'auth'})
        }
