...
exports.getProfile = async (req, res) => {
    try {
        ...
        const [user, address, order, wishlist, wallet] = await Promise.all([userPromise, addressPromise, orderPromise, wishlistPromise, walletPromise]);
        ...
        if (address && order) {
            ...
            if (address.addresses.length > 0) {
                ...
                const orders = order
                return res.render('profile', { user, adresses, orders, wishlist, wallet })
            }
        ...
        if (address) {
            if (address.addresses.length > 0) {
                const adresses = address.addresses
                return res.render('profile', { user, adresses, wishlist, wallet })
            }
        }

        if (order) {
            const orders = order
            return res.render('profile', { user, orders, wishlist, wallet })
        }
